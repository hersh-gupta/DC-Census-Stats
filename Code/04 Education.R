...
#Get variables
variables <- tibble(name = c("Total", "Less than high school graduate", "High school graduate (includes equivalency)", "Some college or associate's degree", "Bachelor's degree", "Graduate or professional degree"),
                    id = c("B06009_001","B06009_002","B06009_003","B06009_004","B06009_005","B06009_006"))
#Get shapefiles
...
acs_merged <- acs %>%
  ...
  pivot_longer(cols = 3:7) %>% select(GEOID,name,value) %>% mutate(name = factor(name, levels = variables$name[-1]))
...
p <- dc_sf %>%
  ...
  labs(title = "Educational Attainment by Ward", fill = "", 
...
p
#Save
ggsave(p, filename = here::here("Output","Education.png"), device = "png", dpi = 320, 
       width = 8, height = 4.5, scale = 1.35)
